...
class DistanceConstraint(SemiInfiniteConstraintInterface):
    def __init__(self,robot,obstacle,I,linkgeoms):
        self.linkgeoms = linkgeoms
        self.obstacle = PenetrationDepthGeometry(obstacle)
        self.I = I
        self.robot = robot
        self.link = robot.link(I[3])
    def setx(self,T):
        self.robot.setConfig(T)
        for i in range(len(self.linkgeoms)):
            if self.linkgeoms[i]:
                self.linkgeoms[i].setTransform(self.linkgeoms[i].getTransform())
    def value(self,T,I):
        worldpt = self.link.getWorldPosition(self.I[5])
        dist = vectorops.distance(self.I[1],worldpt)
        return dist
    def domain(self):
...
class RobotConfigObjective(ObjectiveFunctionInterface):
    def __init__(self,robot,qdes,weight=1.0):
        ...
        self.weight = weight
    def value(self,q):
        if len(q) > len(self.qdes):
            return self.weight*self.robot.distance(q[:len(self.qdes)],self.qdes)**2
        else:
...
class RobotTrajectoryCollisionConstraint(SemiInfiniteConstraintInterface):
    ...
    def minvalue(self,x,bound=None):
        ...
        for i in range(len(traj.milestones)-1):
            ...
            active = dict()
            for (link,env),d in list(di.items()):
                if (link,env) in dn:
                    ...
                    if dlow < dmin:
                        active[link,env] = dlow
            for (link,env),d in list(dn.items()):
            ...
            if len(active) > 0:
                if self.verbose>=2: 
                    ...
                    for (link,env),d in list(active.items()):
                        if d < alinks[link]:
                            alinks[link] = d
                        if d < aenvs[env]:
                            aenvs[env] = d
        ...
        while len(edgequeue) > 0:
            ...
            active = dict()
            for (link,env),d in list(dlow.items()):
                if d >= dmin: continue
                geom = self.trajectory.kinematics.geometry[link]
...
def makeCollisionConstraints_new(obj,envs,gridres_robo=0,pcres_robo=0,gridres_env=0,pcres_env=0):
    """Makes the most efficient collision checker between the given object (RobotModel,
    RobotModelLink, list of RobotModelLinks, RigidObjectModel, or Geometry3D)
    and one or more static geometries.
    ...
    for i,e in enumerate(envs):
        if isinstance(e,PenetrationDepthGeometry):
            ...
            if isinstance(e,Geometry3D):
                ...
                envgeoms[i] = e.geometry()
                print(("  Converting environment item",e.getName()))
            envgeoms[i] = PenetrationDepthGeometry(envgeoms[i],gridres_env,pcres_env)
    if isinstance(obj,RobotModel):
        #everything shares the same robot cache
        cache = RobotKinematicsCache(obj,gridres_robo,pcres_robo)
        return makeCollisionConstraints_new(cache,envs,gridres_robo,pcres_robo,gridres_env,pcres_env)
    elif isinstance(obj,RobotKinematicsCache):
        #everything shares the same robot cache
        robot = obj.robot
        res = []
        ...
        cache = RobotKinematicsCache(obj.robot(),gridres_robo,pcres_robo)
        if TEST_PYCCD:
            return [RobotLinkConvexCollisionConstraint(obj,e,cache) for e in envgeoms],[(obj,e) for e in envs]
        else:
            return [RobotLinkCollisionConstraint(obj,e,cache) for e in envgeoms],[(obj,e) for e in envs]
        ...
        assert all(isinstance(o,RigidObjectModel) for o in obj),"If a list of objects is given, it must be a list of RobotModelLinks"
        if len(obj) == 0: return []
        cache = RobotKinematicsCache(obj[0].robot(),gridres_robo,pcres_robo)
        res = []
        pairs = []
        for o in obj:
            if TEST_PYCCD:
        ...
        geom = PenetrationDepthGeometry(obj.geometry(),gridres_env,pcres_env)
        if TEST_PYCCD:
            return [ObjectConvexCollisionConstraint(geom,e) for e in envgeoms],[(obj,e) for e in envs]
        else:
            return [ObjectCollisionConstraint(geom,e) for e in envgeoms],[(obj,e) for e in envs]
        ...
        assert isinstance(obj,Geometry3D),"obj must be a robot, link, rigid object, or geometry"
        geom = PenetrationDepthGeometry(obj,gridres_env,pcres_env)
        if TEST_PYCCD:
            return [ObjectConvexCollisionConstraint(geom,e) for e in envgeoms],[(obj,e) for e in envs]
        else:
            return [ObjectCollisionConstraint(geom,e) for e in envgeoms],[(obj,e) for e in envs]
...
def optimizeCollFree(obj,env,Tinit,Tdes=None,verbose=1,settings=None):
    ...
    if TEST_PYCCD:
        ...
        #print "IS THIS CONSTRAINT SEMI INFINITE?",isinstance(constraint,SemiInfiniteConstraintInterface)
